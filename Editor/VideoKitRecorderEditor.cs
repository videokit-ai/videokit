    ...
    internal sealed class VideoKitRecorderEditor : Editor {
        ...
        private void OnEnable () {
            var propertyNames = new [] {
                @"format", @"destination", @"videoMode", @"audioMode", @"resolution", @"customResolution",
                @"orientationMode", @"aspectMode", @"cameras", @"texture", @"cameraManager",
                @"frameSkip", @"watermarkMode", @"watermark", @"watermarkRect", @"audioManager",
                @"configureAudioManager", @"audioDeviceGain", @"OnRecordingCompleted",
            };
        ...
        public override void OnInspectorGUI () {
            ...
            if (videoOptions) {
                ...
                if (videoMode != VideoMode.None) {
                    // Video resolution
                    var orientation = (OrientationMode)Enum.GetValues(typeof(OrientationMode)).GetValue(properties[@"orientationMode"].enumValueIndex);
                    ...
                    EditorGUILayout.PropertyField(properties[@"orientationMode"]);
                    if (orientation == OrientationMode.MatchScreen)
                        EditorGUILayout.PropertyField(properties[@"aspectMode"]);
                    // Video input
                    switch (videoMode) {
