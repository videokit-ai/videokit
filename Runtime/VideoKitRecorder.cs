...
namespace NatML.VideoKit {
    ...
    public sealed partial class VideoKitRecorder : MonoBehaviour {
        ...
        private (int width, int height) CreateVideoFormat () => resolution.FrameSize(customResolution)
            .EnsureOrientation(orientationMode)
            .EnsureAspect(aspectMode.EnsureOrientation(orientationMode))
            .EnsureEven();
        ...
        private IDisposable CreateVideoInput () => videoMode switch { // no side effects
            ...
            VideoMode.Texture       => new CustomTextureInput(textureInput, clock, _ => texture) { frameSkip = frameSkip },
