...
namespace NatML.VideoKit {
    ...
    public sealed class VideoKitRecorder : MonoBehaviour {
        ...
        private IMediaRecorder CreateRecorder () {
            ...
            switch (format) {
                case Format.MP4:    return new MP4Recorder(width, height, frameRate, sampleRate, channelCount);
                case Format.HEVC:   return new HEVCRecorder(width, height, frameRate, sampleRate, channelCount);
                case Format.WEBM:   return new WEBMRecorder(width, height, frameRate, sampleRate, channelCount);
