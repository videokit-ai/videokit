...
namespace NatML.VideoKit {
    ...
    public sealed partial class VideoKitRecorder : MonoBehaviour {
        ...
        public OrientationMode orientationMode = OrientationMode.MatchScreen;
        ...
        public AspectMode aspectMode = AspectMode.Resolution;
        ...
        private (int width, int height) CreateVideoFormat () {
            var aspect = aspectMode.EnsureOrientation(orientationMode);
            var (width, height) = resolution.FrameSize()
                .EnsureOrientation(orientationMode)
                .EnsureAspect(aspect)
                .EnsureEven();
            return (width, height);
        }
        ...
        private (int sampleRate, int channelCount) CreateAudioFormat () => audioMode switch {
            ...
            AudioMode.AudioDevice                               => (audioManager?.audioDevice?.sampleRate ?? 0, audioManager?.audioDevice?.channelCount ?? 0),
            _                                                   => (0, 0),
        };
        ...
        private IMediaRecorder CreateRecorder () {
            ...
            return CreateRecorder(width, height, sampleRate, channelCount);
        }
        ...
        private RecorderTextureInput CreateTextureInput () => new RecorderTextureInput(recorder, new [] { // no side effects
            new WatermarkTextureInput(null as IMediaRecorder) {
        ...
        private IDisposable CreateVideoInput () => videoMode switch { // no side effects
            VideoMode.Screen        => new ScreenInput(textureInput, clock),
            VideoMode.Camera        => new CameraInput(textureInput, clock, cameras),
            VideoMode.Texture       => textureInput,
            VideoMode.CameraDevice  => new CameraDeviceInput(textureInput, clock, cameraManager),
            _                       => null,
        };
