...
namespace NatML.VideoKit {
    ...
    public sealed class VideoKitCameraManager : MonoBehaviour, IVideoKitDeviceManager<CameraDevice> {
        ...
        public void StopRunning () {
            if (cameraDevice?.running ?? false) // CHECK // Remove once NatDevice fixes error on WebGL
                cameraDevice?.StopRunning();
            output?.Dispose();
            output = null;
        }
        ...
        private static CameraDevice GetDefaultCameraDevice (Facing facing) {
            var query = new MediaDeviceQuery(MediaDeviceCriteria.CameraDevice, configureAudioSession: false);
