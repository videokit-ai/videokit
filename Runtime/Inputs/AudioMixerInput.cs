...
namespace NatML.VideoKit.Inputs {
    ...
    internal sealed class AudioMixerInput : IDisposable { // INCOMPLETE // Sync
        ...
        public AudioMixerInput (IMediaRecorder recorder, IClock clock, VideoKitAudioManager audioManager, AudioListener audioListener) {
            ...
            audioManager.OnSampleBuffer.AddListener(OnDeviceSampleBuffer);
        ...
        /// <param name="audioDevice">Audio device to record microphone audio from.</param>
        /// <param name="audioListener">Audio listener to record game audio from.</param>
        public AudioMixerInput (IMediaRecorder recorder, VideoKitAudioManager audioManager, AudioListener audioListener) : this(recorder, default, audioManager, audioListener) { }

        /// <summary>
        /// Stop the recorder input and release resources.
        /// </summary>
        public void Dispose () {
            AudioMixerInputAttachment.Destroy(attachment);
            audioManager.OnSampleBuffer.RemoveListener(OnDeviceSampleBuffer);
        }
        #endregion
        ...
        private void OnDeviceSampleBuffer (SampleBuffer sampleBuffer) {
            signal.Signal(deviceFence);
            if (!signal.signaled)
                return;
            lock (deviceFence)
                deviceRingBuffer.Write(sampleBuffer.audioBuffer.sampleBuffer);
            MixBuffers();
        }
