...
+ Added `StructuredOutput` attribute for making `struct` and `class` types compatible with `MediaAsset.Parse`.
...
+ Fixed `MediaRecorder` failing to record with `WAV` audio format on iOS and macOS.
